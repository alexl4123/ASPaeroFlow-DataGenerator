...
and diffs that against a committed baseline. It is not a unit-test suite: it makes no
claim about whether the output is *correct*, only about whether it is *the same*.

The baseline in `baseline/` was captured on `fix/rotation-continuity`, after the
rotation-continuity fix. Everything before that fix was captured on
`refactor/modular-stages-and-checks` (tip `d887bc9`), and the fix deliberately changed
...
## Does the published dataset regenerate from source? **No longer — and that is the point.**
...
Up to `ed42d99` all 400 files matched byte for byte. Since the rotation-continuity fix they
do not, because the published instances were generated by the code that broke rotations:

```
files compared : 400
added          : 0
removed        : 0
changed        : 111  (111 content/shape, 0 row-order-only)
RESULT: MISMATCH
```

| file | instances changed (of 40) |
|---|---|
| `flights.csv` | 32 |
| `mappings/id_maps.json` | 32 |
| `airplanes.csv` | 25 |
| `airplane_flight_assignment.csv` | 22 |

`graph_edges.csv`, `sectors.csv`, `navaid_sector_assignment.csv`, `airports.csv`,
`mappings/vertex_map.csv` and `transform_manifest.json` are **unchanged in all 40**: the fix is
confined to stage 04 and what stage 05 derives from it. The eight instances whose `flights.csv`
still matches are the ones small enough that no leg ever failed to fit the window.

So `--shipped-check` now reports a real difference rather than a regression, and it will keep
doing so until the dataset is regenerated and re-released. **Do not "fix" it by loosening the
comparison.** It is the honest statement that `release_upload/` predates the fix, and the
published data contains the broken rotations `check_instances.py`'s `D8`/`F10` reports. Takes
~50 s.
